			...
			<?php if(!empty($items[0])){ ?>
				<?php for ($i=0; $i < count($items); $i++) { ?>
					<li class="ui-state-default">
						<span class="content"><?php echo $items[$i] ?></span> 
						<span class="delete-item-list-sortable">x</span>
						<span class="edit-item-list-sortable"><i class="fa fa-pencil"></i></span>
					</li>
				<?php } ?>
			<?php } ?>
		</ul>
...
<script>
	$(document).on('click', '.add-sortable-list', function(event) {
		...
		if (text != "") {
			$('#sortable_' + component).append('<li class="ui-state-default"><span class="content">'+ text +'</span> <span class="delete-item-list-sortable">x</span><span class="edit-item-list-sortable"><i class="fa fa-pencil"></i></span></li>');
		}	
	...
	$(document).on('click', '#sortable_<?= $field['complete_name'] ?> .edit-item-list-sortable', function(event) {
		event.preventDefault();
		var content = $(this).parent('li').find('.content').text();
		$('input[name="<?= $field['complete_name'] ?>"]').val(content);
		$(this).parent('li').remove();
	});
